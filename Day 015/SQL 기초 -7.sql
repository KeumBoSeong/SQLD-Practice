...
# 주민번호 앞자리
SELECT RPAD('991111-', 14,'*') FROM DUAL;
...
# SYSDATE 와 ADD_MONTHS 함수를 사용하여 현재 날짜와 6개월 후 날짜가 출력되도록 작성
SELECT SYSDATE, 
...
SELECT EMPNO, ENAME, HIREDATE,
       ...
       TRUNC (MONTHS_BETWEEN (SYSDATE, HIREDATE)) AS MONTH
FROM EMP;
...
SELECT SYSDATE,
       ...
       ROUND(SYSDATE, 'Q') AS FORMAT_Q,  #현재 달의 1일 날짜
...
# 문자열과 숫자를 더하여 출력 # 문자열과 숫자열을 더하면 Error!!
SELECT 'ABCD' + ENMPNO, EMPNO, ENAME FROM EMP
WHERE ENMAE = 'KING';
...
# 문자 데이터를 날짜 데이터로 변환하는 함수 TO_DATE
SELECT TO_DATE('2023-07-17', 'YYYY-MM-DD') AS DATE1
FROM DUAL;

# 문자열을 숫자타입으로 변환 하려면 TO_NUMBER
SELECT 1300 - '1500', '1300' + 1500 FROM DUAL;
# Error Type
SELECT '1,300' - '1,500' FROM DUAL;
SELECT '1300' - '1500' FROM DUAL; # 가능
SELECT TO_NUMBER('1,300', '999,999') - TO_NUMBER('1,500', '999,999') AS RESULT
FROM DUAL; # 가능

# TO_CHAR, TO_NUMBER, TO_DATE << 꼭 기억 해야 할 함수!!
